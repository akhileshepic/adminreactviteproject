...
const SubCategory = () => {
    ...
    const fetchall = async () => {
        try {
            const response = await getAll('subcategory');
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = subcategory.map((product) => product.id);
    ...
    const handleDelete = (id) => {
        ...
        if (confirmDelete) {
            setSubcategory(subcategory.filter((product) => product.id !== id));
        }
    };
    return (
        <div>
            <div className="TableItem w-full px-6  py-6 border rounded-md mt-6 bg-white">
                <h1 className='mb-3 font-bold text-[20px]'>Sub Category List</h1>
                <div className='flex items-center justify-between pb-4 pr-2'>

                    <div className='export w-[25%] ml-auto flex gap-2'>
                                ...
                                <TableRow>
                                    <TableCell padding="checkbox">
                                        <Checkbox
                                            color="primary"
                                            indeterminate={
                                                selected.length > 0 && selected.length < subcategory.length
                                            }
                                            checked={subcategory.length > 0 && selected.length === subcategory.length}
                                                ...
                                                <TableCell>
                                                    <div className="flex flex-wrap gap-2">
                                                        {product?.category.map((item, index) => (
                                                            <span
                                                                key={index}
                                                                className="px-4 py-1 bg-orange-300 text-black text-sm rounded-lg shadow hover:bg-orange-400 cursor-pointer"
                                                            >
                                                                {item.name}
                                                            </span>
                                                        ))}
